 ...
 * 
 * This program is used to isolate and record pulses resulting from an alpha particle passing through our scintillator detector
 * 
...
const int detector_1 = A2; // Detector WITH Alpha Source
const int detector_2 = A3; // Detector WITHOUT Alpha Source
...
int pulse_id = 0; //Records the current number of a pulse
...
void setup()
{
  ...
  adc->setReference(ADC_REFERENCE::REF_3V3); //Sets the reference voltage, default REF_3V3
                                             //*If altered, you must change the conv variable to match
...
void loop() 
{
  while(true)
  {
    ...
    {
      if(adc->analogRead(detector_2) < 2*cutoff/3)
      {
        for(int i=0; i<num_samples; i++)
        {
          data_points[i] = adc->analogRead(detector_1); //sample the alpha source
        }
        if(adc->analogRead(detector_2) < 2*cutoff/3) //make sure that there really isn't a coincidence event happening...
        { 
          PORTB |= (1<<4);
